...
// to utc_offset_hours=0 regardless of the installer machine's actual timezone,
// silently skipping sensor reads for however many hours UTC's 6am-8pm window
// didn't line up with local daytime. The firmware field is whole hours only
// (-12..14), so this rounds; that's a firmware limitation, not fixable here.
export const localUtcOffsetHours = (dateFactory = () => new Date()) => {
  const offsetMinutes = dateFactory().getTimezoneOffset()
  // The "|| 0" avoids returning -0 (e.g. for UTC itself) — harmless once
  // JSON-serialized, but not worth leaking into application logic.
  return Math.max(-12, Math.min(14, Math.round(-offsetMinutes / 60))) || 0
}
