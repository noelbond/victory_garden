...
typedef struct {
    ...
    absolute_time_t next_wait_log_at;
} time_sync_runtime_t;
...
void vg_time_sync_set_epoch_us(uint32_t sec, uint32_t usec) {
    ...
    printf("[time] sync received epoch=%lu\n", (unsigned long)sec);
    stdio_flush();
}
...
void time_sync_init(void) {
    ...
    g_time_sync.next_wait_log_at = make_timeout_time_ms(5000);
    printf("[time] sntp started server=%s\n", VG_DEFAULT_NTP_SERVER);
    stdio_flush();
}
...
void time_sync_poll(void) {
    ...
    // sntp itself has no "failed" callback — the lwIP SNTP client just
    // retries silently on its own schedule. This is the only visibility
    // into "did DNS resolution or the NTP request ever land" versus a
    // firmware hang elsewhere.
    if (g_time_sync.initialized && !g_time_sync.synced &&
        absolute_time_diff_us(get_absolute_time(), g_time_sync.next_wait_log_at) <= 0) {
        printf("[time] still waiting for NTP sync (server=%s)\n", VG_DEFAULT_NTP_SERVER);
        stdio_flush();
        g_time_sync.next_wait_log_at = make_timeout_time_ms(5000);
    }
}
