...
bool sht40_probe(const node_config_t *config) {
    ...
    if (i2c_write_timeout_us(i2c1, SHT40_I2C_ADDRESS, &command, 1, false, SHT40_I2C_TIMEOUT_US) != 1) {
        return false;
    }
    sleep_ms(SHT40_SERIAL_DELAY_MS);
    if (i2c_read_timeout_us(i2c1, SHT40_I2C_ADDRESS, response, sizeof(response), false, SHT40_I2C_TIMEOUT_US) != (int)sizeof(response)) {
        return false;
    }
...
uint8_t sht40_scan_bus(const node_config_t *config) {
    ...
    i2c1_bus_init(config);
    printf("[i2c] scanning i2c1 sda=GP%u scl=GP%u\n",
        (unsigned)config->ads1115_i2c_sda_gpio,
        (unsigned)config->ads1115_i2c_scl_gpio);

    for (uint8_t address = 0x08u; address <= 0x77u; ++address) {
        int rc = i2c_write_timeout_us(i2c1, address, &probe_byte, 1, false, SHT40_I2C_TIMEOUT_US);
        if (rc != 1) {
            continue;
        }
